...
@mcp.tool()
def observe(text: str, session: str | None = None, turn: int = 0) -> dict:
...
@mcp.tool()
def veto(tool: str, args: str = "") -> dict:
    """Veto decision (allow/ask/deny) for a proposed tool call."""
    return runtime().veto(tool, args)


@mcp.tool()
def teach(statement: str, weight: float = 0.5, scope: str = "*", polarity: float = 1.0) -> dict:
...
@mcp.tool()
def dream(min_evidence: int = 2) -> dict:
...
@mcp.tool()
def ratify(proposal_id: str, weight: float = 0.5) -> dict:
...
@mcp.tool()
def selfplay(rounds: int = 3) -> dict:
...
@mcp.tool()
def shadow_review(draft: str) -> dict:
    """Artifact Shadow: critique a draft; only confirmed (costly) signals can veto."""
    return runtime().shadow_review(draft)


@mcp.tool()
def arbitrate(positions: list, candidates: list, candidate_axes: dict | None = None) -> dict:
...
@mcp.tool()
def monitor(text: str, project: str = "") -> dict:
    """Scan the live buffer for synchronicity crossings (distant + cross-domain + archetypal)."""
    return runtime().monitor_scan(text, project or None)


@mcp.tool()
def monitor_outcome(crossing_id: str, useful: bool = True) -> dict:
    """Label a crossing useful/useless — calibrates the monitor's precision."""
    return runtime().monitor_label(crossing_id, useful)


@mcp.tool()
def identity() -> dict:
    """Canonical identity export (Character Ledger is the authority)."""
    return runtime().identity_export()


@mcp.tool()
def memory_eml(out_dir: str, limit: int = 0) -> dict:
    """Export the sint-memory chain as EML-IPC `.eml` (format convergence)."""
    return runtime().memory_to_eml(out_dir, limit=limit or None)


@mcp.tool()
def cache() -> dict:
    """Prompt-cache hit rate measured from the host (prefix stability telemetry)."""
    return runtime().cache_stats()


@mcp.tool()
def poll_events(since: int = 0, kinds: list[str] | None = None, limit: int = 200) -> dict:
    """Drain bus events newer than `since`."""
    return {"events": runtime().events(since=since, kinds=kinds, limit=limit)}


@mcp.tool()
def outcome(node_ids: list[str], useful: bool = True) -> dict:
